...
class PurchaseController extends Controller
{
    ...
    public function success($item_id)
    {
        ...
        $existingTransaction = Transaction::where('product_id', $item->id)->first();

        if ($existingTransaction) {
            if (
                (int) $existingTransaction->buyer_id === (int) $user->id ||
                (int) $existingTransaction->seller_id === (int) $user->id
            ) {
                return redirect()
                    ->route('trades.show', $existingTransaction->id);
            }
